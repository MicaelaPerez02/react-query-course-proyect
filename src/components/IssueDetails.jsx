...
function useIssueData(issueNumber) {
  return useQuery(["issues", issueNumber], ({ signal }) => {
    return fetch(`https://ui.dev/api/courses/react-query/issues/${issueNumber}`, { signal }).then((res) => res.json());
  });
}

function useIssueComments(issueNumber) {
  return useQuery(["issues", issueNumber, "comments"], ({ signal }) => {
    return fetch(`https://ui.dev/api/courses/react-query/issues/${issueNumber}/comments`, { signal }).then((res) =>
      res.json()
    );
  });
}
